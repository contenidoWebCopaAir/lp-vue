...
s.linkInternalFilters="javascript:,copaair.com"
...
function s_doPlugins(s) {
	...
	if(!s.campaign)
		s.campaign=s.getQueryParam('d1'); 
    	
		s.campaign = s.getValOnce(s.campaign,'cname',0);
		
		var temphr=s.getTimeParting('h','-5','2008'); // Set hour 
		var tempday=s.getTimeParting('d','-5','2008'); // Set day
		var tempweek=s.getTimeParting('w','-5','2008'); // Set Weekend / Weekday
		
		if(temphr)
			s.prop17=s.eVar25= tempweek + " : " + tempday + " : " + temphr;
		
		s.prop23=s.eVar27=s.getNewRepeat();
...
s.visitorNamespace="copaairlines"
s.dc=122
/*UNCOMMENT THE LINES OF CODE BELOW AFTER THE SSL CERTIFICATE FOR FIRST PARTY COOKIE HAS BEEN INSTALLED*/
s.trackingServer="somni.copaair.com"
s.trackingServerSecure="omni.copaair.com"
